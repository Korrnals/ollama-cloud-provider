...
#   L2 Sigstore    — keypair mode (cosign.key/cosign.pub), no OAuth browser flow
...
# Cosign L2 runs in keypair mode (cosign.key/cosign.pub): `--key cosign.key`
# selects the keypair so cosign never enters keyless mode (which would
# trigger an interactive OAuth browser flow and hang a non-interactive
# terminal). COSIGN_PASSWORD must be present in the env when the keypair is
# encrypted. Degrades gracefully if cosign is absent or signing fails — L2
# is optional; L1 SHA256 + L3 GPG remain the required layers.
if command -v cosign >/dev/null 2>&1; then
  ...
  # v0.13.0 hang fix (2026-08-20): an encrypted cosign.key without
  # COSIGN_PASSWORD makes cosign prompt on the terminal — in a
  # background/piped run there is no TTY reader, so cosign waits
  # FOREVER (observed: 33 min hang, pipeline killed manually). The
  # "L2 optional → degrade" contract must not silently produce an
  # UNSIGNED release when the operator intended a signed one, so the
  # missing password is a HARD FAIL, not a warn-and-continue. Operators
  # who genuinely want an unsigned L2 release unset neither — they
  # remove cosign from PATH or run with SIGNED_L2=0.
  elif [ -f cosign.key ] && [ -z "${COSIGN_PASSWORD:-}" ]; then
    fail 4 "cosign.key exists but COSIGN_PASSWORD is empty — cosign would hang on the key passphrase prompt. Export COSIGN_PASSWORD (or remove cosign from PATH to skip L2 explicitly)."
  else
    # L2 is optional per script contract — degrade on any cosign failure
    # (missing/bad keypair, passphrase error, network) rather than aborting
    # the whole release. Uses --key cosign.key (keypair mode, no OAuth) and
    # --bundle (new format); the legacy --output-signature flag is
    # deprecated and fails on recent cosign with "must specify --bundle
    # with --new-bundle-format". `< /dev/null` on both invocations is
    # defence-in-depth — even a misbehaving cosign that ignores
    # COSIGN_PASSWORD gets EOF on the prompt and fails fast instead of
    # hanging.
    VSIX_BUNDLE="${VSIX_FILE}.sigstore.bundle"
    SHA_BUNDLE="${SHA256_FILE}.sigstore.bundle"
    if cosign sign-blob --yes --key cosign.key "$VSIX_FILE" --bundle "$VSIX_BUNDLE" < /dev/null 2>/tmp/cosign-vsix.err; then
      COSIGN_OK=1
      ARTIFACTS+=("${VSIX_BASENAME}.sigstore.bundle|L2 sigstore bundle — VSIX")
    ...
    if cosign sign-blob --yes --key cosign.key "$SHA256_FILE" --bundle "$SHA_BUNDLE" < /dev/null 2>/tmp/cosign-sha.err; then
      ARTIFACTS+=("sha256.txt.sigstore.bundle|L2 sigstore bundle — checksums")
...
# ─── Step 6: SBOM ──────────────────────────────────────────────────────────
# Node.js inline generator is PRIMARY: it reads package.json + package-lock.json
# and emits SPDX-2.3 with correct npm dependencies. syft mis-identifies this
# project as .NET (picks up Microsoft.VisualStudio.Extensions.Signing.Reference)
# and omits npm deps, so it is opt-in only via SBOM_USE_SYFT=1.
step 6 "Generate SBOM (SPDX-JSON)"
SBOM_FILE="$RELEASES_DIR/sbom.spdx.json"
if [ "${SBOM_USE_SYFT:-0}" = "1" ] && command -v syft >/dev/null 2>&1; then
  if [ "$DRY_RUN" -eq 1 ]; then
    echo "  syft requested (SBOM_USE_SYFT=1) — would run: syft . -o spdx-json=$SBOM_FILE"
  else
    syft . -o spdx-json="$SBOM_FILE" || fail 6 "syft SBOM generation failed"
  fi
  SBOM_MODE="syft (opt-in via SBOM_USE_SYFT=1)"
  ok "SBOM generated by syft (opt-in)"
else
  if [ "${SBOM_USE_SYFT:-0}" = "1" ] && ! command -v syft >/dev/null 2>&1; then
    warn "SBOM_USE_SYFT=1 set but syft not installed — falling back to Node.js generator."
    warn "Install: https://github.com/anchore/syft"
  fi
  if [ "$DRY_RUN" -eq 1 ]; then
    echo "  would run: node inline generator → $SBOM_FILE (package.json + lockfile)"
  else
...
const sbom = {
  ...
  creationInfo: {
    ...
    creators: ['Tool: run-release-local.sh (node SBOM generator)'],
...
console.log('  node SBOM packages: ' + packages.length);
NODE_EOF
  fi
  SBOM_MODE="node (package.json + lockfile)"
  ok "SBOM generated by inline Node.js generator"
fi
